...
def timeing():
    try:
        ...
        with open("timing.txt", "a+") as f:
            f.write(f"frame={frame_id}, lines={lines}, time={timee}\n")
        return json.dumps({"ok": True})
    except Exception as e:
        print("[ERROR in /timeing]", e)
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(processes = multiprocessing.cpu_count()) as pool:
        ...
        numbers = [len(frame) for frame in frame_latex]
        times = [1.7e-6 * (n ** 2) for n in numbers]
        ...
        top_15_with_indexes = sorted_numbers[:30]

        print("\nTop 30 heaviest frames by line count:")
        new_frame_latex = []
        for index, number in top_15_with_indexes:
            print(f"Frame {index} has {number} lines, ",end="")

            splited = [frame_latex[index][i:i + MAX_LINES] for i in range(0, len(frame_latex[index]), MAX_LINES)]
            print(len(splited), f" <- segments for frame {index}")

            # new_frame_latex.extend(splited)

        # Replace the full frame list with new chunked version
        # frame_latex = new_frame_latex

        # Final estimated time summary
        print('\nPreprocessing complete')
        print(f'--> Total frames to render: {len(frame_latex)}')
        print(f'--> Total expressions: {sum(numbers)}')
        print(f'--> Estimated rendering time: {total_estimated_time:.2f} seconds ({total_estimated_time/60:.2f} minutes)')
